...
def algorithm0(qc: QuantumCircuit, basis_gates, cbits: ClassicalRegister):
	'''fake_johannesburg-0,fake_almaden-3,fake_singapore-0,fake_singapore-3,fake_tokyo-4,fake_hanoi-0,fake_cairo-2,fake_mumbai-0,fake_mumbai-2,fake_mumbai-3,fake_kolkata-3,fake_auckland-1,fake_montreal-0,fake_montreal-2,fake_paris-1,fake_paris-2,fake_toronto-2,fake_cambridge-2,fake_melbourne-3,fake_rochester-0,fake_rochester-1'''
	CX
	P2
...
def algorithm2(qc: QuantumCircuit, basis_gates, cbits: ClassicalRegister):
	...
	CX
	P2
	with qc.if_test((cbits[5], 0)) as else0_:
		pass
	with else0_:
		X0
...
def algorithm3(qc: QuantumCircuit, basis_gates, cbits: ClassicalRegister):
	...
	CX
	P2
	with qc.if_test((cbits[21], 0)) as else0_:
		P2
		with qc.if_test((cbits[21], 0)) as else1_:
			pass
		with else1_:
			CX
	with else0_:
...
def algorithm4(qc: QuantumCircuit, basis_gates, cbits: ClassicalRegister):
	...
	CX
	P2
	with qc.if_test((cbits[24], 0)) as else0_:
		pass
	with else0_:
		X0
